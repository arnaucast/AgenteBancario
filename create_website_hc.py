...
def main():
    ...
    try:
        ...
        name = get_name_for_web(selected_main_cif.strip())
    ...
    if st.session_state.chat_history:
        ...
        if last_message["role"] == "assistant" and (st.session_state.banking_context.traducciones["TAREA_REALIZADA_PROS"] in last_message["content"]
            ...
            if st.session_state.waiting_for_task_confirmation: #in case it comes from rag, no do action and another task is awaiting
                last_user_message["content"]  = "okay"
        ...
        if st.session_state.task_solved:
            ...
            if feedback or st.session_state.task_success:
                ...
                if st.session_state.task_success:
                    ...
                    print(len(st.session_state.separated_tasks))
                    if (hasattr(st.session_state, 'separated_tasks') and 
                        ...
                        print("AAAAAAAAAAAAAAAAAAAAAAAAA52")
                        ...
                        if next_task:
                            ...
                            st.session_state.task_solved = False
                            print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA53")
                    else:
                        print("AAAAAAAAAAAAAAAAAAAAA6")
                        st.session_state.chat_history_agent.append({
                            "role": "assistant",
                            "content": f"Task completed. Summary: {st.session_state.context_summary}"
                        })
                    ...
                    print("AAAAAAAAAAAAAAAAAAAAA7")
                    if st.session_state.Usuario_click == "SI":
                        print("AAAAAAAAAAAAAAAAAAAAA8")
                        st.session_state.Usuario_click = ""
                        st.session_state.current_task = st.session_state.next_task
                        if st.session_state.context_summary != "":
                            tarea = "Este usuario ha tenido anteriormente esta conversación: " + st.session_state.context_summary + "Utilizála como contexto para la siguiente tarea: " + st.session_state.current_task
                        else:
                            tarea = st.session_state.current_task
                        st.session_state.current_agent, st.session_state.is_question_rag, st.session_state.current_rag_info, st.session_state.current_rag_research = configure_agent_coordinator(tarea)
                        ...
                        st.session_state.waiting_for_rag_confirmation = False
                        del st.session_state.next_task
                    elif st.session_state.Usuario_click == "NO" and not st.session_state.waiting_for_rag_confirmation:
                        print("AAAAAAAAAAAAAAAAAAAAA9")
                        st.session_state.Usuario_click = ""
                        st.session_state.chat_history.append({
                            "role": "assistant",
                        ...
                        st.session_state.waiting_for_task_confirmation = False
                        del st.session_state.next_task
                    elif st.session_state.Usuario_click == "NO" and  st.session_state.waiting_for_rag_confirmation:
                        print("AAAAAAAAAAAAAAAAAAAAA10")
                        st.session_state.Usuario_click = ""
                        st.session_state.task_solved = False
                        ...
                        if len(st.session_state.separated_tasks) > 1:
                            st.session_state.waiting_for_task_confirmation = True
                        else:
                            st.session_state.waiting_for_task_confirmation = False
    ...
    if st.session_state.processing_message and st.session_state.chat_enabled:
        ...
        st.session_state.processing_message = None
        print("AAAAAAAAAAAAAAAAAAAAA12")

        try:
            if st.session_state.current_agent is not None:
                print("AAAAAAAAAAAAAAAAAAAAA13")
                current_agent = st.session_state.current_agent
                if isinstance(current_agent, dict):
                    current_agent = current_agent.get('handler', define_default_agent())
                conversation_history, response, task_success = process_task_with_threading(
                ...
                print("AAAAAAAAAAAAAAAAAAAAA14")
                if st.session_state.context_summary != "":
                    user_input_guardr = "Este usuario ha tenido anteriormente esta conversación: " + st.session_state.context_summary + "Utilizála como contexto para esta nueva petición que viene ahora, no devuelves el contexto. Petición: : " + user_input
                    guardrail_result = process_guardrail_with_threading(banking_guardrail_agent, user_input_guardr)
                else:
