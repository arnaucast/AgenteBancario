...
CATEGORY_SEPARATOR_PROMPT = (
    """You are a precise agent that processes bank clients requests with the following specific instructions:

    Specific Matching Guidelines:
    - Find parts of the text where the client references specific categories
    - Don't return general things like "night transactions", "general transacions", etc.. only focus on specific categories

    Example 1:
    Input: "Quiero que me analices las compras de supermercado y los gastos que hago en medicación"
    Output: ["compras de supermercado","Medicación"]
    ...
    Example 3:
    Input: "Give me an analysis of my night-time transactions"
    Output: []"""
)
...
CATEGORY_FILTRATOR = (
    ...
    - Output: ['NOMINA', 'RETIRADA EFECTIVO']. Nomina because sueldo (salarios recibidos" is related to NOMINA and RETIRADA EFECTIVO becasue of Retiradas de efectivo)
...
@function_tool
def execute_complex_query(ctx: RunContextWrapper['BankingContext'],code: str) -> any:
    """
    Execute a multi-line Python code snippet with robust result handling.
    ...
    data = pd.DataFrame(ctx.context.data)
    # Create a local namespace with the dataframe
    local_namespace = {'table': data}  # 'data' assumed to be available globally or passed
...
bank_query_agent = Agent[BankingContext](
...
- importe: Transaction amount in €, dtype = float64
...
analyze_user_data = Agent(
    name="analyze_user_data",
    instructions="""
    You are given a bank user request and the data you can use to answer the request.
